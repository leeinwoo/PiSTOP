...
def card_share():
    for j in range(player): # 플레이어 카드 분배
        ...
        #print("player%d card list : %s" % (j+1, player_card[j]))
        #print("\n")
    #print ("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n")
...
def throw_card():
    ...
    if(get_throw_card==13): # 와일드 카드를 냈을 경우
        ...
        #player_card[player_turn].remove(get_throw_card) # 낸 카드 삭제
        print("\n카드를 뒤집었습니다\n")
        card_sort()
        throw_card()
    else: # 와일드 카드가 아닌 경우
        ...
        if(hand_count==3): # 같은 카드가 3장인 경우
            ...
            if(match_count==1): # 바닥에 일치하는 카드가 있는 경우 - 흔들면서 폭탄
                ...
                player_get_card[player_turn].append(temp_shake.pop())
                player_get_card[player_turn].append(temp_shake.pop())
...
def choice_shake(player_num):
    is_shake = False
    is_shake = input("흔들겠습니까? 예(1), 아니오(0) : ")
    is_shake = int(is_shake)
    if(is_shake == 1):
...
def select_go_stop(player_num):
    global player_prev_score, player_score
    
    #select = input("GO/STOP 여부를 결정하세요 - GO(1), STOP(0) :") #original code
    #select = int(select) #original code
    select = 0 # test code
    if(select==1):
        print("      고!!\n")
        ...
        if(player_score[player_turn] >= 3): # 플레이어 점수가 3점 이상이면
            if(player_go_count[player_turn] >= 1): # 고 상태일 경우(이전 점수랑 비교해서 1점이상 획득해야 고/스톱 가능)
                if(player_score[player_turn] - player_prev_score[player_turn] >= 1):
                    select_go_stop(player_turn) # 고/스톱 여부 결정
            else: # 고 상태가 아닐 경우
                select_go_stop(player_turn) # 고/스톱 여부 결정
        player_turn += 1
