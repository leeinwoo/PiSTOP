...
def create_label():
    ...
    global throwLabel
    throwLabel = Label(main_frame, bg=bg_color)
    throwLabel.pack(expand=1)
...
def update_card(card_list):
    ...
    for i in range(len(card_list)):
        ...
        cardLabels[i].grid(row=4-int(i/4), column=i%4)
        cardLabels[i].bind("<Button-1>",onclick)
...
def onclick(event):
    global card
    print("x: %d, \t y: %d" % (event.x_root, event.y_root))
    for i in range(len(card)):
        if(event.widget==cardLabels[i]):
            ...
            print(cardLabels[i])
            clear()
            get_throw_card(card[i])

#get throw card
def get_throw_card(card):
    global throwLabel
...
init_gui()
card=[15, 17, 27, 37, 47, 125]
update_card(card)
#clear()
#select_card(15,17, 27)
#clear()
#select_answer(1)
root.mainloop()
